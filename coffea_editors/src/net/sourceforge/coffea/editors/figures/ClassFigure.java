...
extends InterfaceFigure<IClassService<?, ?>> {
	...
	public ClassFigure(IClassService<?, ?> srv) {
		...
		remove(methodFigure);
		propertiesFigure = 
			new PropertiesFigure(typeService.getPropertiesServices());
		// Not really clean but need to do this stuff to avoid hell in 
		// the interfaces heap (UML2 model package)
		List<IClassService<?, ?>> nestedClassesServices = 
			new ArrayList<IClassService<?, ?>>();
